...
const Transactionstatuscard: React.FC<TransactionstatuscardProps> = ({ backgroundColor, caption, count, borderColor }) => {
  return (
    <div className={`${backgroundColor ? backgroundColor : 'bg-white'} rounded-md border-2 ${borderColor ? borderColor : 'border-white'} flex flex-col p-4 gap-5`}>
      <div className='text-xs font-[500] text-textcolor'>{caption}</div>
      <div className='text-3xl font-[600] text-textcolor pb-12'>{count}</div>
    </div>
  );
};
